...
from product.module_managers import image_manager
...
@csrf_exempt
def get_list_item_optimized(request):
    ...
    product_name = get_data.get("product_name")
    offset = int(get_data.get("offset"))
    limit = int(get_data.get("limit")) + 6
    shops = get_data.get("shop")
    ...
    image_manager.update_score_with_image(list_item=list_json_return, keyword=product_name)

    return append_header_with_cors(
        JsonResponse({"list_product": search_manager.get_return_order_with_static_score(
                list_json_return)[:limit-6], "total": len(results)}))
